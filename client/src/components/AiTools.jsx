  ...
  return (
    <div className='px-4 sm:px-20 xl:px-32 my-24'>
        <div className='text-center max-w-2xl mx-auto'>
            ...
            <p className='text-gray-500 max-w-lg mx-auto'>Explore a suite of AI tools designed to enhance your creative process and boost productivity.</p>
        </div>
        <div className='flex flex-wrap justify-center mt-10'>
            {AiToolsData.map((tool, index)=>(
                <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition-all duration-300 cursor-pointer' onClick={()=> user && navigate(tool.path)}>
                    <tool.Icon className='h-12 w-12 text-white rounded-xl'  style={{background: `linear-gradient(to bottom, ${tool.bg.from}, ${tool.bg.to})`}}/>
